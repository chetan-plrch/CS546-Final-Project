...
const exportedMethods = {
  ...
  validate(username, firstName, lastName, email, password, gender, city, state, age, isAnonymous, profilePic, connections, isActive){
    ...
    if (typeof profilePic !== 'string' || !profilePic.trim().length) {
  ...
  compareAndUpdate(newDoc, oldDoc){
    ...
    for (let key of oldDocKeys) {
      ...
      } else if (key === 'username' || key === 'firstName' || key === 'lastName' || key === 'email' || key === 'gender' || key === 'city' || key === 'state' || key === 'age' || key === 'isAnonymous' || key === 'role' || key === 'profilePic') {
        if (newDoc[key] !== oldDoc[key]) {
          return false;
        }
      }
